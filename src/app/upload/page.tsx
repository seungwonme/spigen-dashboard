...
function ingest(type: FileType, rawRows: Record<string, unknown>[]): { inserted: number; skipped: number } {
  if (type === "sp") {
    ...
    upsertRows("ad_campaigns", rows, (r) => `${r.date}|SP|${r.campaign_id}`);
  ...
  if (type === "sb") {
    ...
    const skipped = rawRows.length - rows.length;
    upsertRows("ad_campaigns", rows, (r) => `${r.date}|SB|${r.campaign_id}`);
    return { inserted: rows.length, skipped };
  }
  if (type === "sd") {
    ...
    const skipped = rawRows.length - rows.length;
    upsertRows("ad_campaigns", rows, (r) => `${r.date}|SD|${r.campaign_id}`);
    return { inserted: rows.length, skipped };
  }
  if (type === "attribution") {
    ...
    upsertRows("attribution", parsed, (r) => `${r.date}|${r.campaign_id}|${r.product_asin}|${r.publisher}`);
      ...
      <div className="flex justify-between items-start">
        <div>
          ...
          <p className="text-sm text-neutral-500 mt-1">
            CSV 또는 XLSX 파일을 드래그하거나 선택하세요. 모든 데이터는 브라우저 localStorage에 저장됩니다.
          </p>
        </div>
        <button
